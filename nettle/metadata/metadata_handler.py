...
class MetadataHandler:
    ...
    STATION_METADATA_FILE_NAME = "stations.geojson"
    ...
    def get_dict(self, dict_folder, dict_name: str = None):
        ...
        if static_dict is None:
            raise FileNotFoundError(
                f"[metadata_handler.get_dict] could not find {file_path}")
        return static_dict
    ...
    def get_old_metadata(
        ...
        if old_metadata is None:
            self.log.warn(
                f"[metadata_handler.get_old_metadata] could not find an old metadata")
        else:
            self.log.info(
                f"[metadata_handler.get_old_metadata] old metadata retrieved")
        return old_metadata
    ...
    def get_old_station_geo_metadata(
        ...
        old_station_metadata = self.get_old_station_geo_metadata_by_store(
            station_id)
        ...
        if old_station_metadata is None:
            self.log.warn(
                f"[metadata_handler.get_old_station_geo_metadata] could not find an old station metadata")
        else:
            self.log.info(
